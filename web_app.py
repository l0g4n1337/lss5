...
class IndexHandler(tornado.web.RequestHandler):
    ...
    async def prepare(self):
        ...
        if cells:
            ...
            self.text = f"<p style=\"font-size:30px\">Bots Disponibili:</p>{style}\n<table>{cells}</table>"

    def get(self):
        self.write(f"{self.text or 'olá :]'}<br><p><a href=\"https://github.com/zRitsu/DC-MusicBot-RPC/releases\" "
                   f"target=\"_blank\">Scarica l'app Rich Presence qui.</a></p><br>Link da aggiungere nell'app RPC "
                   f"qui di seguito:<p style=\"color:blue\">{self.ws_url}</p><br>")
...
class WebSocketHandler(tornado.websocket.WebSocketHandler):
    ...
    def on_message(self, message):
        ...
        if not ws_id:
            ...
            if not bot_id:
                print(f"disconnessione: per mancanza di user id {self.request.remote_ip}\nData: {data}")
                self.close(code=1005, reason="Disconnessione: per mancanza di ID utente")
                return
            ...
            for ws in users_ws:
                try:
                    ...
                    print(f"Errore durante l'elaborazione dei dati RPC per gli utenti [{', '.join(ws.user_ids)}]: {repr(e)}")
        ...
        if is_bot:
            print(f"Nuova connessione - Bot: {ws_id} {self.request.remote_ip}")
            self.bot_id = ws_id
            bots_ws.append(self)
            return
        ...
        print("\n".join(f"Nuova connessione - Utente: {u} | {data}" for u in self.user_ids))

        for w in bots_ws:
            try:
                w.write_message(json.dumps(data))
            except Exception as e:
                print(f"Errore durante l'elaborazione dei dati da RPC a Bot {w.bot_id}: {repr(e)}")
    ...
    def on_close(self):
        ...
        if self.user_ids:
            print("\n".join(f"Connessione terminata - Utente: {u}" for u in self.user_ids))
            users_ws.remove(self)
            return

        if not self.bot_id:
            print(f"Connessione terminata - IP: {self.request.remote_ip}")

        else:

            print(f"Connessione terminata - Bot: {self.bot_id}")
            ...
            for w in users_ws:
                try:
                    ...
                    print(f"Errore durante l'elaborazione dei dati RPC per gli utenti: [{', '.join(w.user_ids)}]: {repr(e)}")
...
class WSClient:
    ...
    async def connect(self):
        ...
        print(f"{self.bot.user} - Client RPC connesso")
        ...
        while True:
            ...
            try:
                ...
                if isinstance(e, aiohttp.WSServerHandshakeError):
                    print(f"{self.bot.user} - Server offline, tentativo di riconnettersi al server RPC in {int(self.backoff)} secondi.")
                else:
                    print(f"{self.bot.user} - Riconnessione al server RPC in {int(self.backoff)} secondi.")
