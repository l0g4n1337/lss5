...
class ErrorHandler(commands.Cog):
    ...
    async def process_interaction_error(self, inter: disnake.AppCmdInter, error: Exception):
        ...
        if not error_msg:
            ...
            embed.title = "Si è verificato un errore col comando:"
    ...
    async def on_report_submit(self, inter: disnake.ModalInteraction):
        ...
        try:
            ...
            await inter.send("Il messaggio originale è stato eliminato o è inaccessibile.", ephemeral=True)
            return

        if not original_message.embeds:
            await inter.send(
                embed=disnake.Embed(
                    title="Il messaggio incorporato è stato rimosso!",
        ...
        if image_url and not URL_REG.match(image_url):
            ...
            await inter.send(
                embed=disnake.Embed(
                    title="Link immagine non valido!",
                    description=inter.text_values["error_details"]
                ), ephemeral=True
            )
            return
        ...
        await inter.send(
            embed=disnake.Embed(
                description="**Errore segnalato correttamente!**",
                color=self.bot.get_color(inter.guild.me)
            ), ephemeral=True
        )
        ...
        embed.set_author(name=f"Errore segnalato: {inter.author} - {inter.author.id}", icon_url=user_avatar)
