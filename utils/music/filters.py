...
class InvalidFilterArgument(WavelinkException):
    """Un argomento non valido è stato passato a un filtro."""
    pass
...
class AudioFilter:
    ...
    def timescale(cls, speed: float = 1.0, pitch: float = 1.0, rate: float = 1.0):
        ...
        if speed == 0:
            raise InvalidFilterArgument("Timescale speed deve essere più di 0.1")
        if pitch == 0:
            raise InvalidFilterArgument("Timescale pitch deve essere più di 0.1")
        if rate == 0:
            raise InvalidFilterArgument("Timescale rate deve essere più di 0.1")
    ...
    def tremolo(cls, frequency: float = 2.0, depth: float = 0):
        ...
        if frequency <= 0:
            raise InvalidFilterArgument("Tremolo frequency deve essere più di 0")

        if not 0.1 < depth < 1.1:
            raise InvalidFilterArgument("Tremolo frequency deve essere tra 0,1 e 1.0")
    ...
    def vibrato(cls, frequency: float = 2.0, depth: float = 0):
        ...
        if frequency <= 0:
            raise InvalidFilterArgument("Vibrato frequency deve essere più di 0.")

        if not 0.1 < depth < 1.1:
            raise InvalidFilterArgument("Vibrato frequency deve essere tra 0.1 e 1.0")
