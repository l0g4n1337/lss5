...
async def process_spotify(bot: BotCore, requester: disnake.Member, query: str, *, hide_playlist=False):
    ...
    if url_type == "album":
        ...
        data["playlistInfo"]["name"] = "Il piú riprodotto: " + [a["name"] for a in result[0].artists if a.id == url_id][0]
        data["tracks"] = result
        ...
        raise GenericError(f"**Link spotify non riconosciuto/supportato:**\n{query}")
...
def spotify_client(config: dict) -> Optional[Client]:
    try:
        ...
        print(
            f"Spotify APIKEY non configurato correttamente nelle variabili host (o file .env), "
            f"controlla e riprova se vuoi il supporto musicale di Spotify (Errore: {repr(e)}).\n{'-'*30}")
        return