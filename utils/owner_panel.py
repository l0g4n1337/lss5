...
class PanelView(disnake.ui.View):
    ...
    async def opts_callback(self, interaction: disnake.MessageInteraction):
        ...
        self.embed.description = txt or "Comando eseguito con successo!"
        await interaction.response.edit_message(embed=self.embed, view=self)

    async def interaction_check(self, interaction: disnake.MessageInteraction):

        if not (await self.bot.is_owner(interaction.user)):
            await interaction.send("Solo i miei sviluppatori possono utilizzare queste opzioni.", ephemeral=True)
            return
    ...
    async def on_error(self, error: Exception, item: disnake.ui.Item, interaction: disnake.MessageInteraction):
        self.embed.description = parse_error(interaction, error) or "**C'è stato un errore:**\n" \
